...
<AccordionGroup>
  <Accordion title="Bước 1 — Trích nghĩa vụ từ HĐ">
    Robot `legal-obligation-extractor` đọc HĐ → liệt kê **chính xác**: ai phải trả ai, bao nhiêu, hạn nào, phạt nếu trễ.
    
    Output: JSON các nghĩa vụ + ảnh chụp trang HĐ tương ứng (làm evidence).
  </Accordion>
  <Accordion title="Bước 2 — L1 Nhắc nợ (tone nhẹ)">
    Robot `legal-debt-warning-letter` branch `nhac_no` xuất 1 letter ~1 trang:
  ...
  <Accordion title="Bước 3 — L2 Cảnh cáo (cite điều luật)">
    Robot `legal-debt-warning-letter` branch `canh_cao`:
    - Tone: nghiêm khắc
  ...
  <Accordion title="Bước 4 — L3 Trước khởi kiện (neutral)">
    Robot `legal-debt-warning-letter` branch `truoc_khoi_kien`:
  ...
  <Accordion title="Bước 5 — Đơn khởi kiện">
    Nếu vẫn không trả, robot `legal-tranh-tung-drafter` xuất Đơn KK đúng **Mẫu 01-DS BLTTDS 2015**.
    
    Trước khi nộp Tòa: robot `legal-pleading-validator` audit 6 mục (thẩm quyền, thời hiệu, yêu cầu KK, án phí...).
  </Accordion>
</AccordionGroup>
