...
<AccordionGroup>
  <Accordion title="Bước 1 — Audit 6 mục">
    ...
    - HĐ MB / NDA / HĐ Vay → `contract-auditor` (umbrella)
    - HĐ DV → `service-contract-auditor`
    - HĐ Thuê → `rental-contract-auditor`
    - HĐLĐ → `employment-contract-auditor`
    - HĐ Vay riêng → `loan-contract-auditor`
  ...
  <Accordion title="Bước 2 — Redline counter-proposal">
    Robot `contract-redliner` lấy audit issues + stance của bạn → xuất file redline với marker:
  ...
  <Accordion title="Bước 3 — Benchmark nhiều vendor">
    Nếu có 2-5 HĐ vendor cùng category → robot `benchmark-comparator` so sánh 6 trục:
...
<CardGroup cols={2}>
  ...
  <Card title="Memo gửi sếp" icon="file-pen">
    (Optional) `memo-drafter` tóm tắt rủi ro + action items
  </Card>
</CardGroup>
...
- Sau negotiate, ký xong → track via `matter-tracker`
- Tạo `obligation-extractor` để bóc nghĩa vụ → assign team
