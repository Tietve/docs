...
<AccordionGroup>
  <Accordion title="Bước 1 — Audit HĐLĐ trước">
    Robot `employment-contract-auditor` kiểm 6 mục:
  ...
  <Accordion title="Bước 2A — TB chấm dứt (hết hạn / đơn phương / tinh giản)">
    Robot `employment-termination-drafter` umbrella:
  ...
  <Accordion title="Bước 2B — QĐ kỷ luật sa thải">
    Robot `disciplinary-decision-drafter`:
  ...
  <Accordion title="Bước 3 — Audit policy (NQLĐ)">
    Robot `policy-auditor` check NQLĐ DN có:
...
- Update NQLĐ nếu có gap → `internal-rules-drafter`
